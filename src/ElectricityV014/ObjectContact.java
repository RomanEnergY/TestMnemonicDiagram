        ...
        for (Contact contactConnect : this.contactConnectArrayList) {
            ...
            if (voltageTerminal != null) {
                ...
                if (!voltageTerminal.getContact().equals(contactConnect)) {
                    // Проверяем на изменение класса напряжения
                    if (voltageTerminal.getEventElectricity().equals(pathElectricCurrentClone.getEventElectricity())) {
                        // если класс напряжения не изменился
                        PathElectricCurrent pathElectricCurrentVoltageTerminalClone = pathElectricCurrentClone.getClone();
                        pathElectricCurrentVoltageTerminalClone.addContactToStart(voltageTerminal.getContact());
                        voltageTerminal.getContact().addPathElectricCurrents(pathElectricCurrentVoltageTerminalClone);

                        voltageTerminal.getContact().electricityOn(pathElectricCurrentVoltageTerminalClone);
                    }
                    else {
                        // если класс напряжения изменился
                        PathElectricCurrent pathElectricCurrentVoltageTerminalClone = pathElectricCurrentClone.getClone();
                        pathElectricCurrentVoltageTerminalClone.addContactToStart(voltageTerminal.getContact());
                        voltageTerminal.getContact().addPathElectricCurrents(pathElectricCurrentVoltageTerminalClone);
...
//        System.out.println(this.mainContact + " this.contactConnectArrayList: " + this.contactConnectArrayList);
        for (Contact contactConnect : this.contactConnectArrayList) {
//            contactConnect.electricityOff(pathElectricCurrent);
            contactConnect.removePathElectricCurrents(pathElectricCurrent);
...
//                System.out.println("voltageTerminal:" + voltageTerminal);
                // если объект electricityObject не ссылается на contactConnect, источник подачи напряжения
//                voltageTerminal.getContact().electricityOff(pathElectricCurrent);
                if (!voltageTerminal.getContact().equals(contactConnect)) {
...
//        System.out.println(pathElectricCurrent);
    }
}
