...
public class Generator implements ElectricityObject {
    ...
    @Override // ElectricityObject
    public void mapConnectionContactElectricityObject(EventElectricity eventElectricity, Contact contact) {
    ...
    void run(){
        ...
        if (this.aGeneratorRun) { // Если генератор запущен ранее --> выдаем ошибку
            ...
            // передаем событие о появлении напряжения от генератора на контакт генератора №1
            this.objectContact_p1.getContact().electricityOn(this.eventElectricity);
    ...
    void stop(){
        ...
        if (this.aGeneratorRun) { // Если генератор запущен ранее --> останавливаем
            ...
            this.objectContact_p1.getContact().electricityOff(this.eventElectricity);
        }
        else { // Если генератор не включен, включаем
