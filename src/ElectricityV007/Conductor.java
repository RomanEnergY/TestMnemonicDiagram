    ...
    public void mapConnectionContactElectricityObject(EventElectricity eventElectricity, Contact contact) {
        if (contact.equals(objectContact_p1.getContact())) { // если появилось напряжение на контакте №1
            if (this.aBoolean_p1_p2) { // если объект включен
                // передаем событие о появлении напряжения на контакте №2
                this.objectContact_p2.getContact().electricityOn(eventElectricity);
            }
        }
        else if (contact.equals(objectContact_p2.getContact())) { // если появилось напряжение на контакте №2
            if (this.aBoolean_p1_p2) { // если объект включен
                // передаем событие о появлении напряжения на контакте №2
                this.objectContact_p1.getContact().electricityOn(eventElectricity);
            }
