...
public class ObjectContact implements ElectricityConnection, ElectricityConsumer {
     ...
     *
     * @param eventElectricity событие появления напряжения от источника
     */
...
//        System.out.println(this.getClass() + " " + this.contactConnectArrayList + " eventElectricity:" + eventElectricity);
        for (Contact contactConnect : this.contactConnectArrayList) {
            contactConnect.getLineElectricity(eventElectricity, contactStartEventElectricity);
            VoltageTerminal voltageTerminal = contactConnect.getVoltageTerminal(eventElectricity);
            if (voltageTerminal != null) {
                if (voltageTerminal.getContact().equals(contactStartEventElectricity))
                    break;

                if (!voltageTerminal.getContact().equals(contactConnect))
                    voltageTerminal.getContact().electricityOn(voltageTerminal.getEventElectricity(), contactStartEventElectricity);
            }
        }
//
//
...
//        for (Contact contactConnect: this.contactConnectArrayList) {
//            contactConnect.electricityOn(eventElectricity);
//        }
    }
    ...
    public void electricityOff(EventElectricity eventElectricity) {
        ...
        for (Contact contactConnect : this.contactConnectArrayList) {
            contactConnect.electricityOff(eventElectricity);
        }
    ...
    public void getLineElectricity(ArrayList<PathElectricCurrent> arrayListPathElectricCurrents, ArrayList<Contact> contacts, EventElectricity eventElectricity, Contact contactStartEventElectricity) {
        ...
        boolean b = true;
        for (Contact contactConnect : this.contactConnectArrayList) {
            if (contactConnect == contactStartEventElectricity)
                b = false;
        }

        if (b) {
            contacts.add(this.getMainContact());

            for (Contact contactConnect : this.contactConnectArrayList) {
                ArrayList<Contact> contactsClone = (ArrayList<Contact>) contacts.clone();
                if (contactConnect.getElectricityObject().isContactSourceOfVoltage()) {
                    VoltageTerminal voltageTerminal = contactConnect.getVoltageTerminal(eventElectricity);
                    if (voltageTerminal.getEventElectricity().equals(eventElectricity)) {
                        arrayListPathElectricCurrents.add(new PathElectricCurrent(eventElectricity, contactsClone));
                    }
                }

                contactsClone.add(contactConnect);
                Contact contactCurrent = contactConnect.getElectricityObject().getContactElectricCurrent(contactConnect);
//            System.out.println("contactConnect:" + contactConnect +" --> contactCurrent:" + contactCurrent);


                if (contactCurrent != null)
                    contactCurrent.getLineElectricity(arrayListPathElectricCurrents, contactsClone, eventElectricity, contactStartEventElectricity);
            }
        }
    }
}
